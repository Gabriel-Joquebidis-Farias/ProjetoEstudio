...
namespace estudio
{
    class DAO_Conexao
    {
        ...
        public static int login(String usuário, String senha)
        {
            int tipo = 0; 
            try
            {
                con.Open();
                MySqlCommand login = new MySqlCommand("Select * from Estudio_Login where usuario ='" + usuário + "' and senha ='" + senha +"'", con);
